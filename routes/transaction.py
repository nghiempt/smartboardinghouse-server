...
async def get_transaction_by_ID(transaction_ID: int):
    ...
    status_message = HTTP_STATUS_CODE.responses[status_code]
    return ResponseObject(True, status_code, status_message, Transaction.serializeDict(conn.execute(transaction.select().where(transaction.c.ID == transaction_ID)).fetchone()))
